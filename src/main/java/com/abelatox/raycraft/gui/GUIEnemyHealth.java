...
public class GUIEnemyHealth extends Screen {

	public GUIEnemyHealth() {
	...
	public void onRenderOverlayPost(RenderGameOverlayEvent event) {
		...
		if (target == null || event.getType().equals(RenderGameOverlayEvent.ElementType.HEALTH) && event.isCancelable()) {
			...
			if (event.getType() == RenderGameOverlayEvent.ElementType.TEXT) {
				if (player != null && ModCapabilities.get(player) != null) {
					...
					ms.translate(10, screenHeight, 0);
